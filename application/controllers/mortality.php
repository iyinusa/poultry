...
class Mortality extends CI_Controller {
	...
	public function index() {
		...
		if($get_id != ''){
			$gq = $this->user->query_rec_single('id', $get_id, 'bz_mortality');
			foreach($gq as $item){
				$data['e_id'] = $item->id;
		...
		if($del_id != ''){
			if($this->user->delete_rec('id', $del_id, 'bz_mortality') > 0){
		...
		if($_POST){
			$mortality_id = $_POST['mortality_id'];
			...
			if($mortality_id != ''){
				...
				if($this->user->update_rec('id', $mortality_id, 'bz_mortality', $upd_data) > 0){
				...
				if($this->user->reg_rec('bz_mortality', $reg_data) > 0){
					$data['err_msg'] = '<div class="alert alert-info"><h5>Successfully</h5></div>';
				} else {
					$data['err_msg'] = '<div class="alert alert-info"><h5>There is problem this time. Try later</h5></div>';
				}
